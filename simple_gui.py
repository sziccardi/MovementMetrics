...
def display_file_select(chosen_file, existing):
    ...
    window_name = "BMAT Processed Folder Selection" if existing else "Video to Process Selection"
    ...
    if graph_type == GraphType.HEAT_MAP:
        ...
        for key in range(len(data)):
            ...
            for bin_i in range(np_data.shape[1]):
                ...
                rect_top = np_data[key][bin_i]
                if prev_point[0]>0:
                    graph.draw_line(prev_point, (start_point+bin_w*0.5, rect_top), color=colors[key],width=3)
                    #graph.draw_line((start_point, rect_top), (start_point + bin_w, rect_top), color=colors[key],width=3)
                prev_point = (start_point+bin_w*0.5, rect_top)
